...
function NavLink({
  ...
  return (
    ...
    >
      <span className={cn('sidebar-icon-wrap relative shrink-0', active && 'sidebar-icon-wrap-active')}>
        <Icon className="h-[17px] w-[17px]" strokeWidth={active ? 2.25 : 1.85} />
        {collapsed && alertCount !== undefined && alertCount > 0 && (
          <span className="absolute -right-0.5 -top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-danger px-0.5 text-[9px] font-bold text-white ring-2 ring-[--bg-secondary]">
